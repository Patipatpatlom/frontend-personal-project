import { useEffect, useState } from "react";
import axios from "axios";
import { motion, AnimatePresence } from "framer-motion";
import { Sparkles, ShoppingCart, Moon, Sun } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";
import { useCartStore } from "../store/useCartStore";

// Import modular components
import AddCakeForm from "./AddCakeForm";
import EditCakeModal from "./EditCakeModal";
import CakeCard from "./CakeCard";

const API = (import.meta.env.VITE_API_URL || "http://localhost:5000") + "/api/desserts";

export default function FancyShopUI(props) {
  const [desserts, setDesserts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState(null);
  const [dark, setDark] = useState(false);
  const [flying, setFlying] = useState(null); // {x,y,img}
  const [form, setForm] = useState({
    name: "",
    price: "",
    image: "",
    category: "",
  });
  const [editing, setEditing] = useState(null);

  const location = useLocation();
  const navigate = useNavigate();
  const { date, time } = location.state || {};

  // Global Zustand cart store
  const cart = useCartStore((state) => state.cart);

  const role = props.user?.user?.role || "USER";

  const fetchDesserts = async () => {
    try {
      const res = await axios.get(API);
      if (Array.isArray(res.data)) setDesserts(res.data);
      else if (res.data.data) setDesserts(res.data.data);
      else setDesserts([]);
    } catch (err) {
      console.error(err);
      setDesserts([]);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchDesserts();
  }, []);

  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API}/${id}`);
      setToast("Deleted item 💀");
      fetchDesserts();
    } catch (err) {
      console.error(err);
      setToast("Delete failed 💀");
    }
  };

  const addToCart = (item, e) => {
    setToast("Added to cart 🛒");
    navigate("/custom", {
      state: {
        cake: item,
        date,
        time,
      },
    });

    // 🔥 flying animation origin
    const rect = e.currentTarget.getBoundingClientRect();
    setFlying({ x: rect.left, y: rect.top, img: item.image });

    setTimeout(() => setFlying(null), 800);
  };

  const handleAddDessert = async () => {
    if (!form.name || !form.price || !form.image) {
      setToast("Fill all fields 💀");
      return;
    }

    try {
      await axios.post(API, {
        ...form,
        price: Number(form.price),
      });

      setToast("Added 🎂");

      setForm({
        name: "",
        price: "",
        image: "",
        category: "",
      });

      fetchDesserts();
    } catch (err) {
      console.error(err);
      setToast("Add failed 💀");
    }
  };

  const handleUpdateDessert = async () => {
    try {
      await axios.put(`${API}/${editing.id}`, {
        name: editing.name,
        price: Number(editing.price),
      });

      setToast("Updated 🎂");
      setEditing(null);
      fetchDesserts();
    } catch (err) {
      console.error(err);
      setToast("Update failed 💀");
    }
  };

  return (
    <div className={dark ? "dark" : ""}>
      <div
        className="min-h-screen transition-all duration-500 
bg-gradient-to-br 
from-pink-100 via-pink-200 to-pink-300 
dark:from-pink-300 dark:via-pink-200 dark:to-pink-300
p-8
w-full bg-[url('/src/assets/cake.png')] bg-cover"
      >
        {/* HEADER */}
        <motion.div
          initial={{ opacity: 0, y: -30 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex items-center justify-between mb-10 bg-white/40 backdrop-blur-md p-4 rounded-3xl shadow-sm border border-white/20"
        >
          <div className="flex items-center gap-3">
            <Sparkles className="text-pink-500 animate-pulse" />
            <h1 className="text-4xl md:text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-purple-500">
              Cake Shop
            </h1>
          </div>

          <div className="flex items-center gap-4">
            {/* CART */}
            <div className="relative cursor-pointer" onClick={() => navigate("/cart")}>
              <ShoppingCart className="text-pink-600" />
              <span className="absolute -top-2 -right-2 bg-pink-500 text-white text-xs px-2 py-0.5 rounded-full font-bold shadow-md">
                {cart.length}
              </span>
            </div>
          </div>
        </motion.div>

        {/* LOADING */}
        {loading && (
          <div className="flex justify-center my-20">
            <span className="loading loading-spinner loading-lg text-pink-500"></span>
          </div>
        )}

        {/* ADMIN ADD CAKE */}
        {role === "ADMIN" && (
          <AddCakeForm
            form={form}
            setForm={setForm}
            onAdd={handleAddDessert}
            setToast={setToast}
          />
        )}

        {/* GRID OF CAKE CARDS */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {desserts.map((item, i) => (
            <CakeCard
              key={item.id || i}
              item={item}
              i={i}
              role={role}
              onAddToCart={addToCart}
              onEdit={setEditing}
              onDelete={handleDelete}
            />
          ))}
        </div>

        {/* FLYING IMAGE ANIMATION */}
        <AnimatePresence>
          {flying && (
            <motion.img
              src={flying.img}
              initial={{ x: flying.x, y: flying.y, opacity: 1, scale: 1 }}
              animate={{
                x: window.innerWidth - 80,
                y: 20,
                scale: 0.3,
                opacity: 0,
              }}
              exit={{ opacity: 0 }}
              transition={{ duration: 0.8 }}
              className="fixed w-24 h-24 object-cover rounded-xl z-50 pointer-events-none"
            />
          )}
        </AnimatePresence>

        {/* EDIT CAKE MODAL */}
        <AnimatePresence>
          {editing && (
            <EditCakeModal
              editing={editing}
              setEditing={setEditing}
              onSave={handleUpdateDessert}
            />
          )}
        </AnimatePresence>

        {/* TOAST NOTIFICATION */}
        <AnimatePresence>
          {toast && (
            <motion.div
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 50 }}
              className="fixed bottom-5 right-5 bg-black/80 backdrop-blur-md text-white px-4 py-2 rounded-xl shadow-2xl z-50 font-bold border border-white/20"
              onAnimationComplete={() => setTimeout(() => setToast(null), 1500)}
            >
              {toast}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
    </div>
  );
}
