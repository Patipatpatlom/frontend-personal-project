      ...
      <main className="max-w-7xl mx-auto pt-10 px-6 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="sticky top-28 bg-white/80 backdrop-blur-sm rounded-[50px] p-10 border-4 border-white shadow-2xl flex flex-col items-center">
             ...
             <div className="relative group">
                <motion.img 
                    ...
                    src={cake?.image || 'https://images.unsplash.com/photo-1578985543162-ea9956926578?q=80&w=500&auto=format&fit=crop'} 
                ...
                {cakeDecor && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                        ...
                        <p className="text-white font-['Pacifico',cursive] text-2xl drop-shadow-lg -rotate-12 bg-pink-500/20 px-4 py-2 rounded-lg">
                            {selectedPresetText === 'Custom' ? (customText || 'Your Text') : selectedPresetText}
                        </p>
                    </motion.div>
                )}
             </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-[50px] p-10 border-4 border-white shadow-2xl space-y-8">
          <div className="flex items-center justify-between border-b border-pink-100 pb-6">
            <h2 className="text-3xl font-black text-[#83002D] italic uppercase">Customize</h2>
            <div className="text-right">
                <p className="text-xs text-gray-400 font-bold">DATE: {date || "Not Selected"}</p>
                <p className="text-xs text-gray-400 font-bold">TIME: {time || "--:--"}</p>
            </div>
          </div>

          {/* Size Selection */}
          <section>
            <h3 className="font-black text-gray-800 mb-4 uppercase tracking-tighter">1. Select Size</h3>
            <div className="flex gap-3 flex-wrap">
              {sizes.map(size => (
                <button 
                  key={size.value} type="button"
          ...
          <section className="space-y-4">
            <h3 className="font-black text-gray-800 uppercase tracking-tighter">2. Message on Cake</h3>
            <button 
              type="button"
              ...
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${cakeDecor ? 'border-pink-500' : 'border-gray-300'}`}>
                {cakeDecor && <div className="w-3 h-3 bg-pink-500 rounded-full" />}
              </div>
            </button>
          ...
          <div className="pt-8 border-t-2 border-dashed border-pink-100 flex flex-col sm:flex-row items-center justify-between gap-6">
             <div className="flex items-center gap-4 bg-white p-2 rounded-2xl shadow-inner border border-gray-100">
                <button type="button" onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 font-black text-xl hover:text-pink-500">-</button>
                <span className="font-black text-xl w-6 text-center">{quantity}</span>
                <button type="button" onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 font-black text-xl hover:text-pink-500">+</button>
             </div>
             
             <div className="text-center sm:text-right">
                <p className="text-[10px] text-gray-400 uppercase font-black tracking-[0.2em]">Total Amount</p>
                <p className="text-4xl font-black text-[#83002D] italic">{totalPrice.toLocaleString()}.-</p>
             </div>
